...
public static class SchematicaFileFormat
{
    ...
    public static void ExportSchematica(string fileName) {
        ...
        try { 
            ...
            ZipEntry schematicaZipEntry = new ZipEntry("data.dat");
            schematicaZipEntry.DateTime = DateTime.Now;
            outputStream.PutNextEntry(schematicaZipEntry);
    ...
    public static bool CanExportSchematica() {
        ...
        if (!CaptureInterface.EdgeAPinned || !CaptureInterface.EdgeBPinned ||
            !ValidCoordinates(CaptureInterface.EdgeA) || !ValidCoordinates(CaptureInterface.EdgeB))
            return false;

        return true;
    }

    public static SchematicaData ImportSchematica(string fileName) {
        if (String.IsNullOrEmpty(fileName))
